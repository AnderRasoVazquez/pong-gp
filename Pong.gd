...
func _process(delta):
	...
	var left_rect = Rect2(get_node("leftPaddle").get_pos() - pad_size * 0.5 , pad_size)
	var right_rect = Rect2(get_node("rightPaddle").get_pos() - pad_size * 0.5 , pad_size)
	#Ball movement:
	ball_pos += direction * ball_speed * delta
	# Flip when bouncing on roof or floor:
	...
	# Move left pad
	var left_pos = get_node("leftPaddle").get_pos()
	if (left_pos.y > 0 and Input.is_action_pressed("left_move_up")):
	    left_pos.y += -PAD_SPEED * delta
	if (left_pos.y < screen_size.y and Input.is_action_pressed("left_move_down")):
	    left_pos.y += PAD_SPEED * delta
	get_node("leftPaddle").set_pos(left_pos)
